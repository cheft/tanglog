<script lang="ts">
  ...
  let displayPosts = $derived(
    data.posts.map(post => {
      ...
      const content = ($currentLang === 'zh' ? post.zh : post.en) || 
  ...
  <div class="container mx-auto px-6">
    <div class="text-center mb-16">
      <span
        ...
        >{$t.blog.label}</span
      >
      <h1 class="text-3xl md:text-5xl font-bold text-slate-900 mt-2 mb-4">
        {$t.blog.title}
      </h1>
    ...
    {#if displayPosts.length > 0}
      <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        {#each displayPosts as post, i}
          <article
            class="bg-white rounded-3xl overflow-hidden border border-slate-100 hover:shadow-xl hover:translate-y-[-4px] transition-all duration-300 group flex flex-col h-full"
          ...
          >
            ...
            <div class="p-8 flex flex-col flex-grow">
              ...
              <h2 class="text-xl font-bold text-slate-900 mb-3 group-hover:text-primary-700 transition-colors line-clamp-2">
                <a href="{$currentLang === 'zh' ? '/zh' : ''}/blog/{post.slug}" class="focus:outline-none">
                  {post.title}
                </a>
              </h2>

              <p class="text-slate-600 mb-6 line-clamp-3 flex-grow">
                {post.description}
              </p>
              ...
              <div class="flex items-center justify-between pt-4 border-t border-slate-100 mt-auto">
                <a
                  ...
                  class="flex items-center text-primary-600 font-medium hover:text-primary-700 group/link"
                >
                  {$t.blog.readMore}
                  <ArrowRight class="w-4 h-4 ml-2 transform group-hover/link:translate-x-1 transition-transform" />
                </a>
                
                <button class="text-slate-400 hover:text-primary-600 transition-colors" title={$t.blog.share}>
