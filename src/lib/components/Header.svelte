...
<header
  class="fixed top-0 left-0 right-0 z-50 transition-all duration-300 border-none outline-none {isScrolled
    ? 'glass py-4'
    : 'bg-transparent py-6'}"
>
  <div class="container mx-auto px-6 flex items-center justify-between">
    ...
    <a
      href={langPrefix || "/"}
      class="text-2xl font-bold text-slate-900 tracking-tighter flex flex-row gap-2 items-center"
    >
      <svg
        width="40px"
        height="27px"
      ...
      <div
        class="text-transparent bg-clip-text bg-gradient-to-r from-[#294686] to-[#34A67E]"
      >
        {$t.companyName}
      </div>
    </a>
    ...
    <nav class="hidden md:flex items-center space-x-8">
      {#each navLinks as link}
        <a
          href={link.href}
          class="text-sm font-medium text-slate-600 hover:text-slate-900 transition-colors relative group"
        >
          {link.name}
          <span
            class="absolute -bottom-1 left-0 w-0 h-0.5 bg-primary-600 transition-all group-hover:w-full"
          ></span>
        </a>
      {/each}
      ...
      <button
        on:click={toggleLang}
        class="flex items-center space-x-1 text-sm font-medium text-slate-600 hover:text-slate-900 transition-colors"
      >
        <Globe class="w-4 h-4" />
      ...
      <a
        ...
        class="px-5 py-2 rounded-full bg-slate-900 hover:bg-slate-800 text-white text-sm font-medium transition-all shadow-lg shadow-slate-900/20"
      >
        {$t.nav.consult}
      </a>
    </nav>
    ...
    <div class="flex items-center space-x-4 md:hidden">
      <button
        on:click={toggleLang}
        class="text-slate-900 flex items-center space-x-1"
      >
        <Globe class="w-5 h-5" />
        <span class="text-sm font-bold"
          >{$currentLang === "zh" ? "EN" : "中"}</span
        >
      </button>
  ...
  {#if isMobileMenuOpen}
    <div
      transition:slide
      class="md:hidden absolute top-full left-0 right-0 glass border-t border-slate-200/50"
    >
